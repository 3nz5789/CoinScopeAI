...
class BinanceWebSocketClient:
    ...
    async def connect(self):
        """
        Connect to WebSocket API with exponential backoff reconnection.
    ...
    async def _message_handler(self):
        """
        Handle incoming WebSocket messages with auto-reconnect.

        Handles:
        - JSON responses matched by id → resolves pending futures
        - Binance application-level ping frames {"ping": <ts>} → sends pong
        - ConnectionClosed → triggers reconnect with exponential backoff
        """
        backoff = 1
            ...
            try:
                async for raw in self.ws:
                    try:
                        ...
                        logger.warning(f"Non-JSON frame received: {raw[:200]}")
                        continue

                    # Application-level ping from Binance — must pong within 10 min
                    if "ping" in data:
                        pong_payload = json.dumps({"pong": data["ping"]})
                        await self.ws.send(pong_payload)
                        self.last_ping = time.time()
                        logger.debug("Pong sent (server ping)")
                        continue

                    # Regular API response matched by request id
                    if "id" in data:
                        request_id = data["id"]
                ...
                for _req_id, future in list(self.pending_requests.items()):
                    if not future.done():
                ...
                logger.error(f"Message handler error: {e} — reconnecting in {backoff}s")
    ...
    async def _ping_handler(self):
        """
        Monitor that we are receiving server pings within the expected window.

        Binance sends a JSON ping every ~3 minutes; if we haven't seen one in
        9 minutes the connection is likely stale. Log a warning so operations
        ...
        try:
            while self.connected:
                ...
                if self.connected and (time.time() - self.last_ping) > STALE_THRESHOLD_S:
                    logger.warning(
                        "No server ping received in %.0fs — connection may be stale",
