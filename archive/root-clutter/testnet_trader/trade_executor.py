...
class TradeResult:
    ...
    def print_confirmation(self):
        ...
        print("\n" + "═" * 55)
        print(
            f"  CoinScopeAI Trade Confirmation  —  {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}"
        )
        print("═" * 55)
        print(
            f"  Signal      : {side_emoji}  {self.signal.symbol}  (score {self.signal.score:+.1f})"
        )
...
class TradeExecutor:
    ...
    def execute(self, signal: Signal) -> TradeResult:
        ...
        try:
            ...
            logger.info(
                f"  Filters: step={filters['step_size']}  tick={filters['tick_size']}  "
            ...
            logger.info(
                f"  ✅ Entry filled @ ${filled_price:,.4f}  orderId={entry_order['orderId']}"
            )
