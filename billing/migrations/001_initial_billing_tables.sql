...
DO $$ BEGIN
    CREATE TYPE billing.subscription_tier AS ENUM (
        'free', 'trader', 'desk_preview', 'desk_full', 'unknown'
    );
...
-- Source of truth: billing/config.py + CoinScopeAI Track B pricing docs
...
(
    'free', 'Free',
    0, 0,
    3, 240, 5,          -- 3 pairs, 4h scans, 5 alerts/day
    7,                  -- 7-day journal
    FALSE, NULL,        -- no API
...
-- Trader — $79/mo | $758.40/yr  ← Most Popular
(
    'trader', 'Trader',
    7900, 75840,
    25, 60, 50,         -- 25 pairs, 1h scans, 50 alerts/day
    -1,                 -- unlimited journal
    FALSE, NULL,        -- no API (engine API is Desk Preview+)
...
-- Desk Preview — $399/mo | $3,830.40/yr
(
    'desk_preview', 'Desk Preview',
    39900, 383040,
...
-- Desk Full — $1,199/mo | $11,510.40/yr
(
    'desk_full', 'Desk Full',
    119900, 1151040,
    -1, 5, -1,          -- unlimited pairs, 5min scans, unlimited alerts
    -1,
    TRUE, 1000,
    TRUE, TRUE, TRUE, TRUE,
    TRUE, TRUE, TRUE,
