...
class Settings(BaseSettings):
    ...
    max_daily_loss_pct: float = Field(
        5.0, ge=0.1, le=100.0, description="Max daily loss % before trading halts"
    )
    ...
    # ── Stripe Billing ────────────────────────────────────────────────────
    # Track B canonical tiers: Free / Trader / Desk Preview / Desk Full.
    # Price IDs are created by scripts/setup_stripe_test_products.py.
    ...
    # Monthly price IDs
    stripe_price_free_monthly:        str = Field("", description="Stripe Price ID: Free monthly")
    stripe_price_trader_monthly:      str = Field("", description="Stripe Price ID: Trader monthly")
    stripe_price_desk_preview_monthly: str = Field("", description="Stripe Price ID: Desk Preview monthly")
    stripe_price_desk_full_monthly:   str = Field("", description="Stripe Price ID: Desk Full monthly")
    # Annual price IDs
    stripe_price_free_annual:         str = Field("", description="Stripe Price ID: Free annual")
    stripe_price_trader_annual:       str = Field("", description="Stripe Price ID: Trader annual")
    stripe_price_desk_preview_annual:  str = Field("", description="Stripe Price ID: Desk Preview annual")
    stripe_price_desk_full_annual:    str = Field("", description="Stripe Price ID: Desk Full annual")
    # Redirect URLs
    billing_success_url: str = Field(
        "http://localhost:8080/billing_success.html?session_id={CHECKOUT_SESSION_ID}",
        description="Post-checkout success URL"
    )
    billing_cancel_url: str = Field(
        "http://localhost:8080/pricing.html",
        description="Post-checkout cancel URL"
    )
