...
Data Layer
----------
Uses BinanceRESTClient (data/binance_rest.py) for all market data fetches.
This replaces the previous ccxt dependency and ensures:
  - Config-driven testnet/mainnet switching (TESTNET_MODE in .env)
  - Shared rate-limit tracking (X-MBX-USED-WEIGHT-1M headers)
  - Consistent symbol format: BTCUSDT not BTC/USDT
  - Typed error handling (BinanceRESTError, RateLimitError, AuthError)
"""
...
class CoinScopeOrchestrator:
    ...
    def __init__(self, testnet: bool = True, pairs: list = None):
        # Use the native BinanceRESTClient instead of ccxt.
        # Reads testnet_mode from config (settings.testnet_mode) but accepts
        # an explicit override via the testnet parameter for flexibility.
    ...
    def fetch_bars(self, symbol: str, timeframe: str = "4h", limit: int = 300) -> pd.DataFrame:
        """
        Fetch OHLCV bars using the native BinanceRESTClient.

        symbol must be in Binance Futures format: BTCUSDT (not BTC/USDT).
        Returns an empty DataFrame on error so scan_pair can handle it gracefully.
        """
        try:
            # BinanceRESTClient.get_klines is async — run it in the event loop
            raw = asyncio.run(self._rest.get_klines(symbol, timeframe, limit=limit))
    ...
    def scan_pair(self, symbol: str) -> dict:
        ...
        # 4 — Whale block
        # Symbols are now BTCUSDT format (no slash). Strip the USDT suffix.
