...
class PairMonitor:
    ...
    def _save(self) -> bool:
        """Save pair stats atomically. Returns True on success, False on I/O error."""
        import tempfile
        path = self.path
        try:
            ...
            logging.getLogger(__name__).error(
                "pair_monitor: failed to save stats to %s: %s", path, exc
            )
    ...
    def record_trade(self, symbol: str, pnl_pct: float, regime: str, signal_direction: str):
        ...
        if not self._save():
            ...
            logging.getLogger(__name__).warning(
                "pair_monitor: trade for %s recorded in memory but not persisted.", symbol
            )
