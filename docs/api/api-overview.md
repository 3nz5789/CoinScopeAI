...
The API is unversioned today. During validation, paths are considered stable. Once the validation phase exits (target: post-P0, June 2026), any breaking change to a consumed endpoint must add a `/v2/` path rather than mutate the existing one.
