...
FEATURE_COLS = [
    ...
    # Trend
    "adx",
    "di_split",  # primary trend strength features
...
def load_and_split(path: str, test_sessions_per_regime: int = 1):
    ...
    log.info("Dropped %d NaN rows (%.1f%%).", before - len(df), 100 * (before - len(df)) / before)
    ...
    log.info(
        "Train: %d rows (%d sessions) | Test: %d rows (%d sessions)",
        len(df_train),
        df_train[SESSION_COL].nunique(),
        len(df_test),
        df_test[SESSION_COL].nunique(),
    )
...
class RegimeClassifierV3:
    ...
    def __init__(
        self, ensemble: VotingClassifier, le: LabelEncoder, feature_cols: list[str]
    ) -> None:
    ...
    def predict(self, features: dict | pd.Series | np.ndarray) -> dict:
        ...
        if isinstance(features, dict):
            x = np.array([features.get(c, 0.0) for c in self.feature_cols], dtype=float).reshape(
                1, -1
            )
        ...
        prob_dict = {
            str(self.le.inverse_transform([i])[0]): round(float(p), 4) for i, p in enumerate(proba)
        }
        return {
            "regime": regime,
...
def main() -> None:
    ...
    for i, row_label in enumerate(LABEL_ORDER):
        row_str = f"{row_label[:9]:<10}" + "".join(
            f"{cm[i][j]:>11}" for j in range(len(LABEL_ORDER))
        )
    ...
    log.info("Model → %s  (%.1f MB)", MODEL_PKL, os.path.getsize(MODEL_PKL) / 1e6)
    ...
    meta = {
        ...
        "feature_importance": [
            {"feature": f, "importance": round(float(i), 5)} for f, i in importance
        ],
        "models": {
    ...
    for i in range(min(3, len(X_test))):
        result = classifier.predict(dict(zip(FEATURE_COLS, X_test[i])))
        actual = y_test[i]
        match = "✓" if result["regime"] == actual else "✗"
        log.info(
            "  [%s] actual=%-16s pred=%-16s conf=%.3f",
            match,
            actual,
            result["regime"],
            result["confidence"],
