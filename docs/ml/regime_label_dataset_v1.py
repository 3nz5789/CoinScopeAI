...
def gen_mean_reverting(
    n: int, theta: float, mu: float, sigma: float, rng: np.random.Generator
) -> np.ndarray:
    """
...
def gen_volatile(
    n: int, sigma: float, jump_prob: float, jump_size: float, rng: np.random.Generator
) -> np.ndarray:
    """
...
def compute_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ADX(14) with +DI / -DI
    plus_dm = np.where(
        (h - np.roll(h, 1)) > (np.roll(lo, 1) - lo), np.maximum(h - np.roll(h, 1), 0.0), 0.0
    )
    minus_dm = np.where(
        (np.roll(lo, 1) - lo) > (h - np.roll(h, 1)), np.maximum(np.roll(lo, 1) - lo, 0.0), 0.0
    )
    plus_dm[0] = 0
    minus_dm[0] = 0
    ...
    df["vol_zscore"] = np.where(vol_roll_std > 0, (v - vol_roll_mean) / vol_roll_std, 0.0)
    ...
    df["atr_pct_zscore"] = np.where(atp_std > 0, (atr_pct - atp_mean) / atp_std, 0.0)

    bbw_mean = pd.Series(bb_width).rolling(50).mean().values
    bbw_std = pd.Series(bb_width).rolling(50).std().values
    df["bb_width_zscore"] = np.where(bbw_std > 0, (bb_width - bbw_mean) / bbw_std, 0.0)
    ...
    df["price_vs_ema200"] = np.where(
        df["ema_200"].notna(), (c - df["ema_200"].values) / df["ema_200"].values * 100, np.nan
    )
    df["price_vs_ema50"] = np.where(
        df["ema_50"].notna(), (c - df["ema_50"].values) / df["ema_50"].values * 100, np.nan
    )

    # Stochastic %K(14)
    lo_14 = pd.Series(lo).rolling(14).min().values
    hi_14 = pd.Series(h).rolling(14).max().values
    stoch_k = np.where((hi_14 - lo_14) > 0, 100 * (c - lo_14) / (hi_14 - lo_14), 50.0)
    df["stoch_k"] = stoch_k
...
def generate_all_sessions() -> List[pd.DataFrame]:
    ...
    for regime, cfg in REGIME_PARAMS.items():
        ...
        for s_idx, params in enumerate(cfg["sessions"]):
            ...
            df = _make_ohlcv_from_closes(closes, vol_base, vol_noise, sp, t0, regime, rng)
...
def main() -> None:
    ...
    combined = (
        combined.groupby("session_id", group_keys=False).apply(drop_warmup).reset_index(drop=True)
    )
    ...
    log.info(
        "Saved dataset → %s  (%d rows, %d features)", OUTPUT_CSV, len(combined), len(out_cols) - 3
    )
