...
Every deploy to the production-candidate VPS runs through this list. During P0 validation (May 2026), most items below default to "no" — the engine is explicitly frozen.
...
If any box above is checked, close the PR. The change waits until P0 validation completes (§8 Capital Cap criteria met).
...
This block is reference only. Do not execute during P0 validation phase.
