...
for item in [
    ...
    check(
        f"root clean: no {item}",
        not exists,
        "still exists — move to canonical folder" if exists else "",
    )
...
if os.path.isdir(REPO):
    ...
    check(
        ...
        not uncommitted,
        f"{len(uncommitted.splitlines())} files" if uncommitted else "",
    )
else:
...
if os.path.isfile(env):
    ...
    check(
        ...
        "MAX_OPEN_POSITIONS=5" in c,
        "shows =3" if "MAX_OPEN_POSITIONS=3" in c else "not found",
    )
    check(
        "MAX_LEVERAGE=10",
        "MAX_LEVERAGE=10" in c,
        "shows =20" if "MAX_LEVERAGE=20" in c else "not found",
    )
...
try:
    ...
    check(
        ...
        f"got: {data.get('max_open_positions')}",
    )
