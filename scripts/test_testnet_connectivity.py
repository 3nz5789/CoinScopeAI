...
try:
    ...
    ok(
        f"Server time: {datetime.fromtimestamp(server_time/1000, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}"
    )
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if open_pos:
                for p in open_pos:
                    info(
                        f"  {p['symbol']}: {p['positionAmt']} @ {p['entryPrice']} | PnL: {p['unRealizedProfit']}"
                    )
            else:
...
for symbol in SYMBOLS:
    try:
        resp = requests.get(
            f"{TESTNET_REST}/fapi/v1/ticker/price", params={"symbol": symbol}, timeout=10
        )
        if resp.status_code == 200:
...
try:
    ...
    if resp.status_code == 200:
        ...
        for k in klines:
            ...
            info(
                f"  {ts}  O:{float(k[1]):,.1f}  H:{float(k[2]):,.1f}  L:{float(k[3]):,.1f}  C:{float(k[4]):,.1f}  V:{float(k[5]):,.1f}"
            )
    else:
