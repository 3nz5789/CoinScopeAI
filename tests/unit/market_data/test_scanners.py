    ...
    async def test_no_signal_insufficient_data(self, bus):
        ...
        for price in [50000, 50010]:
            event = _make_event(
                ...
                MarkPrice(exchange=Exchange.BINANCE, symbol="BTCUSDT", mark_price=price),
            )
    ...
    async def test_breakout_long_with_oi_expansion(self, bus):
        ...
        for i, oi_val in enumerate([10000, 10300]):
            event = _make_event(
                ...
                OpenInterest(exchange=Exchange.BINANCE, symbol="BTCUSDT", open_interest=oi_val),
                ts=base_ts + i,
            )
            await scanner._on_event(event)
...
class TestFundingExtremeScanner:
    ...
    async def test_high_funding_signal(self, bus):
        ...
        event = _make_event(
            ...
            FundingRate(exchange=Exchange.BINANCE, symbol="BTCUSDT", funding_rate=0.001),
        )
        await scanner._on_event(event)
    ...
    async def test_normal_funding_no_signal(self, bus):
        ...
        event = _make_event(
            EventType.FUNDING_RATE,
            FundingRate(exchange=Exchange.BINANCE, symbol="BTCUSDT", funding_rate=0.0001),
        )
        await scanner._on_event(event)
    ...
    async def test_cross_exchange_divergence(self, bus):
        config = _make_config(
            ...
            thresholds={
                "high_funding_pct": 1.0,
                "low_funding_pct": -1.0,
                "cross_exchange_spread_pct": 0.03,
                "min_exchanges": 2,
            },
        )
        scanner = FundingExtremeScanner(config, bus)
        ...
        event2 = _make_event(
            EventType.FUNDING_RATE,
            FundingRate(exchange=Exchange.BYBIT, symbol="BTCUSDT", funding_rate=-0.001),
        ...
        signals = await scanner.evaluate()
        divergence_signals = [
            s for s in signals if s.signal_type == "funding_cross_exchange_divergence"
        ]
...
class TestSpreadDivergenceScanner:
    ...
    async def test_mark_mid_divergence(self, bus):
        config = _make_config(
            thresholds={"mark_mid_divergence_bps": 10.0, "cross_exchange_divergence_bps": 100.0}
        )
        ...
        event_mp = _make_event(
            EventType.MARK_PRICE,
            MarkPrice(exchange=Exchange.BINANCE, symbol="BTCUSDT", mark_price=50000),
        )
        await scanner._on_event(event_mp)

        event_ob = _make_event(
    ...
    async def test_cross_exchange_mark_divergence(self, bus):
        ...
        event2 = _make_event(
            EventType.MARK_PRICE,
            MarkPrice(exchange=Exchange.OKX, symbol="BTCUSDT", mark_price=50100),
